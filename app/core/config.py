...
class Settings:
    def __init__(self) -> None:
        ...
        self.recall_data_db_path = env(
            "RECALL_DATA_DB_PATH", "data/recall_tools.sqlite3"
        )
        ...
        self.recall_transcript_enabled = env(
            "RECALL_TRANSCRIPT_ENABLED", "true"
        ).lower() in {"1", "true", "yes"}
        self.recall_transcript_language_code = env(
            "RECALL_TRANSCRIPT_LANGUAGE_CODE", "en"
        )
        self.recall_transcript_mode = env(
            "RECALL_TRANSCRIPT_MODE", "prioritize_low_latency"
        )
        self.recall_everyone_left_timeout_seconds = env_int(
            "RECALL_EVERYONE_LEFT_TIMEOUT_SECONDS", 300
        )
        self.recall_everyone_left_activate_after_seconds = env_int(
            "RECALL_EVERYONE_LEFT_ACTIVATE_AFTER_SECONDS", 1
        )
        self.bot_only_cleanup_enabled = env(
            "BOT_ONLY_CLEANUP_ENABLED", "true"
        ).lower() in {"1", "true", "yes"}
        self.bot_only_cleanup_seconds = env_int("BOT_ONLY_CLEANUP_SECONDS", 300)
        self.bot_only_cleanup_interval_seconds = env_int(
            "BOT_ONLY_CLEANUP_INTERVAL_SECONDS", 30
        )
        self.chat_gateway_commands_enabled = env(
            "CHAT_GATEWAY_COMMANDS_ENABLED", "true"
        ).lower() in {
            "1",
            "true",
            "yes",
        ...
        self.recall_video_mixed_layout = env(
            "RECALL_VIDEO_MIXED_LAYOUT", "gallery_view_v2"
        )
        ...
        self.zoom_signed_in_enabled = env(
            "ZOOM_SIGNED_IN_ENABLED", "false"
        ).lower() in {
            "1",
            "true",
            "yes",
        }
        self.zoom_zak_user_id = env(
            "ZOOM_ZAK_USER_ID", self.zoom_create_user_id or "me"
        )
        self.zoom_zak_cache_ttl_seconds = env_int("ZOOM_ZAK_CACHE_TTL_SECONDS", 5400)
        self.zoom_zak_callback_secret = env(
            "ZOOM_ZAK_CALLBACK_SECRET", self.service_api_key
        )
